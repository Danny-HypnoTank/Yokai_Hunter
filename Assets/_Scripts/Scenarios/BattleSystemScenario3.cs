...
public class BattleSystemScenario3 : BattleSystem
{
    ...
    private void Update()
    {
        ...
        if (Enemy4Unit.CurHP == 0)
        {
            Enemy4Unit.Animator.SetInteger("UnitAS", 6);
        }
...
#region Methods
    ...
    private void PlayerTurn() //Setting player turn on transition from any state to player turn
    {
        ...
        Enemy4Unit.CurMoves = Enemy4Unit.MaxMoves;
        ...
        if (Enemy1Unit.CurHP <= 0 && Enemy2Unit.CurHP <= 0 && Enemy3Unit.CurHP <= 0 && Enemy4Unit.CurHP <= 0)
        {
    ...
    public void TargetEnemy1Button() //Button to target enemy 1
    {
        ...
        else //Else Run attack and disable targeting so players can't target multiple enemies
        {
            ...
            Enemy3TargetingUI.SetActive(false);
            Enemy4TargetingUI.SetActive(false);
    ...
    public void TargetEnemy4Button() //Button to target enemy 3
    {
        if (state != BattleState.PLAYERTURN) //Check to make sure it is the players turn
        {
            return;
        ...
        else //Else run attack and disable targeting ui so player can't target more than one enemy
        {
            ...
            Enemy3TargetingUI.SetActive(false);
            Enemy4TargetingUI.SetActive(false);
            StartCoroutine(PlayerAttackEnemy4());
        }
    }
    ...
    private void EndBattle() //End Battle Check
    {
        ...
        {
            PlayerUnit.UnitStats.playerCoins += Random.Range(10, 55);
            PlayerHUD.SetCoins();
...
#region Coroutines
    IEnumerator SetupBattle() //Setting Up the Battle
    {
        ...
        //Instantiate Enemy 4 at Spawnpoint
        GameObject enemy4Go = Instantiate(Enemy4Prefab[Random.Range(0, 2)], Enemy4SpawnPoint);
    ...
    //Player Attacking Enemy 2 Action
    IEnumerator PlayerAttackEnemy4()
    {
        if (PlayerUnit.Focused == true)
        {
            Enemy4Unit.TakeDamage(CurrentCard.Power + PlayerUnit.FocusMultiplier + (PlayerUnit.UnitStats.damageArtefact * 10));
            PlayerUnit.Focused = false;
            PlayerHUD.SetFocus(PlayerUnit);
        }
        else
        {
            Enemy4Unit.TakeDamage(CurrentCard.Power + (PlayerUnit.UnitStats.damageArtefact * 10));
        }
        PlayerUnit.CurMoves -= CurrentCard.ResourceCost;

        PlayerHUD.SetMoves(PlayerUnit);
        Enemy4HUD.SetHP(Enemy4Unit.CurHP, Enemy4Unit);
        DialogueText.text = "The attack is successful!";
        PlayerUnit.Animator.SetInteger("UnitAS", 1);
        Enemy4Unit.Animator.SetInteger("UnitAS", 5);

        yield return new WaitForSeconds(2f);

        PlayerUnit.Animator.SetInteger("UnitAS", 0);
        Enemy4Unit.Animator.SetInteger("UnitAS", 0);
    ...
    //Enemy 4 Turn Pattern
    IEnumerator Enemy4Turn()
    {
        CardManager.TurnEndCardCycle();
        if (Enemy4Unit.CurHP > 0)
        {
            DialogueText.text = Enemy4Unit.UnitName + " attacks!";
            Enemy4Unit.CurMoves -= 1;

            yield return new WaitForSeconds(1f);

            bool isDead = PlayerUnit.TakeDamage(Enemy4Unit.Damage);

            PlayerHUD.SetHP(PlayerUnit.CurHP, PlayerUnit);
            PlayerHUD.SetDefence(PlayerUnit);
            Enemy4HUD.SetMoves(Enemy4Unit);

            PlayerUnit.Animator.SetInteger("UnitAS", 5);
            Enemy4Unit.Animator.SetInteger("UnitAS", 1);
            yield return new WaitForSeconds(2f);
            ...
            else if (!isDead && Enemy4Unit.CurMoves > 0)
            {
                StartCoroutine(Enemy4Turn());
            }
            else if (!isDead && Enemy4Unit.CurMoves <= 0)
