...
public class BattleSystemScenarioTutorial : BattleSystem
{
...
#region Methods
    ...
    private void EndBattle() //End Battle Check
    {
        ...
        {
            PlayerUnit.UnitStats.playerCoins += Random.Range(10, 20);
            PlayerHUD.SetCoins();
...
#region Coroutines
    ...
    IEnumerator PlayerAttackEnemy2()
    {
        if (PlayerUnit.Focused == true)
        {
            Enemy2Unit.TakeDamage(CurrentCard.Power + PlayerUnit.FocusMultiplier);
            PlayerUnit.Focused = false;
            PlayerHUD.SetFocus(PlayerUnit);
        }
        else
        {
            Enemy2Unit.TakeDamage(CurrentCard.Power);
        }
        PlayerUnit.CurMoves -= CurrentCard.ResourceCost;
